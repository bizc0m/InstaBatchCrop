...
Version de developpement basee sur v1.54, compilee et testee localement.
...
- Points et zones d'interet manuels par photo, prioritaires pour l'aperçu et le batch.
- Watermark texte optionnel applique au rendu final.
- Apercu export avec watermark visible et corrections auto-appliquees au batch.
- Interface bilingue FR/EN via un selecteur dans les controles.
- Reglages de deplacement avec icones fleches horizontale/verticale et zoom.
- Icône d'application intégrée dans le bundle macOS `dist/2.0`.
...
1. Lancer l'application autonome `dist/2.0/InstaBatch Crop V2.0.app`, depuis Xcode ou avec `swift run InstaBatchCrop`.
...
5. Choisir `FR` ou `EN` dans `Langue` si besoin.
6. Selectionner une image et cliquer `Actualiser` si l'aperçu n'est pas encore a jour.
7. Optionnel: choisir `Point` ou `Zone` puis marquer les zones prioritaires dans l'apercu avant.
8. Activer l'outil main pour deplacer le cadrage directement dans l'apercu apres, ou ajuster avec les fleches horizontale/verticale et le zoom. Les corrections sont sauvegardees automatiquement pour le batch.
9. Cliquer `Traiter toutes les photos`.
...
3. Donne la priorité aux points/zones marques manuellement, puis pondere les visages plus fortement que corps, animaux, objets et saillance.
...
- outil main avec marge de pan aux bords;
- priorite des points/zones d'interet manuels.
- watermark visible dans l'aperçu export.
...
Resultat local: 19 tests passes.
...
L'icone source est conservee dans `assets/AppIcon.png`. Le bundle macOS utilise `AppIcon.icns` dans `dist/2.0/InstaBatch Crop V2.0.app/Contents/Resources/`.
...
- Les points/zones d'interet et corrections manuelles sont conserves en memoire pendant la session; il n'y a pas encore de fichier projet persistant.

## Plan d'amelioration

1. Ajouter segmentation Vision/Core ML de sujet principal pour des contours plus precis.
2. Exposer un editeur de cadrage visuel avec poignees.
